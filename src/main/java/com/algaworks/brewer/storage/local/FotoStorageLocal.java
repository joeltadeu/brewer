...
public class FotoStorageLocal implements FotoStorage {
	...
	public String salvar(MultipartFile[] files) {
		...
		if (files != null && files.length > 0) {
			...
			try {
				Thumbnails.of(this.local.resolve(novoNome).toString()).size(40, 68)
						.toFiles(Rename.PREFIX_DOT_THUMBNAIL);
			} catch (IOException e) {
				throw new RuntimeException("{upload.foto.exception.erro.gerando.thumbnail}", e);
			}
		}
	...
	public byte[] recuperarFoto(String nome) {
		try {
			...
			throw new RuntimeException("{upload.foto.exception.erro.lendo.foto}", e);
		}
	}
	...
	public byte[] recuperarThumbnail(String nome) {
		...
		return recuperarFoto("thumbnail." + nome);
	}
	...
	private void criarPastas() {
		try {
			...
			throw new RuntimeException("{upload.foto.exception.erro.criando.pasta}", e);
		}
	}
}
