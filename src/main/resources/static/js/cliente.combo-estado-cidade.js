...
Brewer.ComboCidade = (function() {
	...
	function ComboCidade(comboEstado) {
		...
		this.inputHiddenCidadeSelecionada = $('#cidadeIdSelecionada');
	}
	
	ComboCidade.prototype.iniciar = function() {
		reset.call(this);
		this.comboEstado.on('alterado', onEstadoAlterado.bind(this));
		var estadoId = this.comboEstado.combo.val();
		inicializarCidades.call(this, estadoId);
	}
	
	function onEstadoAlterado(evento, id) {
		this.inputHiddenCidadeSelecionada.val('');
		inicializarCidades.call(this, id);
	}
	
	function inicializarCidades(id) {
		if (id) {
			var resposta = $.ajax({
	...
	function onBuscarCidadesFinalizado(cidades) {
		...
		var cidadeIdSelecionada = this.inputHiddenCidadeSelecionada.val();
		if (cidadeIdSelecionada) {
			this.combo.val(cidadeIdSelecionada);
		}
	}
